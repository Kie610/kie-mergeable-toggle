...
トグル対象レンダラーの全頂点デルタを遠方 (`1e6`) にしたブレンドシェイプを生成し、
...
- 生成は `Mesh.AddBlendShapeFrame(name, 100f, deltas(1e6), null, null)`。normal/tangent は null。
  **デルタは有限値で作る**。NaN は Unity が格納時に 0 へ潰し(実測)、`+Infinity` は
  VRChat クライアントの自分視点で描画されなくなる(2026-08-28 実機で特定。下記)
...
bounds を再計算しない。シリアライズされた localBounds をそのまま使う。加えてデルタが
有限なので、**どこかで bounds を再計算されても値は有限のまま**で、カリングされない。

### なぜ `+Infinity` をやめたか (2026-08-28)

∞ デルタを持つメッシュは、**VRChat クライアントの自分視点 (一人称・カメラ) で
レンダラーごと描画されなくなる**。実機で切り分けた結果は次のとおり。

- 素のアバター: 正常 / AAO 単独: 正常 / 本パッケージ単独: **変換対象だけ消える**
- 鏡 (ミラークローン) では完全に正常に見える。Unity エディタ・Av3Emulator でも再現しない
- 隠蔽シェイプのウェイトが 0 (表示状態) でも消える。**メッシュに ∞ デルタが存在するだけで起きる**
- デルタを有限値へ変えたら実機で表示された

ビルド成果物の静的な計測 (localBounds / mesh.bounds / rootBone / UWO / 幾何の収まり /
`m_Enabled` カーブ / シェイプの結合) はいずれも正常で、**手元では検出できない**。
クライアント側の詳細な機序は未特定。∞ を使うかぎり同じ壊れかたを再発させ得るので、
有限値を不変条件とする。
